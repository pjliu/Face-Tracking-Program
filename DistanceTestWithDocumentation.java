...
public class DistanceTestWithDocumentation
{
    public static void main(String[] args) throws IOException 
    {
        ...
        String origImg = "G:\\Documents\\Pictures\\CamTestPics2\\14.jpg";
        String datFile = "G:\\Documents\\Pictures\\CamTestPics2\\14.txt";
...
*/                      
            int colorThreshold = 200;//Used to recognize the infrared dots
            int distThreshhold = 50;//Used as a radius to determine whether certain points belong to the same dot
//          int xsum=0;
//          int ysum=0;
//          int count=0;
            ...
            for (int i=0; i<imgDim.height; i++)
            {
                ...
                {
                    int r = in.read();//reads the r value
                    int g = in.read();//reads the g value
                    int b = in.read();//reads the b value
                    
                    if ((r+g+b) > colorThreshold)//determines whether the specific dot is greater than the threshold
                    {
//                      count++;
//                      xsum += j;
//                      ysum += i;
                        points.add(new Point(j, i));//if it is, then add the point into the arraylist
                    }
/*
...
*/                  
            ...
            for (int k=0; k<groups.length; k++)
            {
                ...
                System.out.println("The midpoint of point ["+(k+1)+"] is at: {"+midpoints[k].x+","+midpoints[k].y+"}");                
            }
            System.out.println("The distance between midpoint0 and midpoint1="+calcDist(midpoints[0], midpoints[1]));
            System.out.println("The distance between midpoint0 and midpoint2="+calcDist(midpoints[0], midpoints[2]));
    System.out.println("The distance between midpoint1 and midpoint2="+calcDist(midpoints[1], midpoints[2])); 
            System.out.println("The distance between the camera and the points is approximately " + runt2(midpoints[0].y, midpoints[1].y, midpoints[2].y) + " inches");
/*            
...
*/
    ...
    public static double runt(int tpy, int lpy, int rpy)
    {        
